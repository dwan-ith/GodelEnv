document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateModeIndicator(mode, provider = null) {
        if (!modeIndicator || !valMode) return;
        modeIndicator.className = "mode-indicator " + mode;
        if (mode === "llm") {
            valMode.innerText = `LLM (${provider || 'unknown'})`;
        } else if (mode === "deterministic") {
            valMode.innerText = "DETERMINISTIC FALLBACK";
        } else if (mode === "error") {
            valMode.innerText = "ERROR";
        } else {
            valMode.innerText = "UNKNOWN";
        }
        lastAgentMode = mode;
    }
    ...
    async function fetchProviderStatus() {
        try {
            ...
            if (response.ok) {
                ...
                updateModeIndicator(hasLlm ? "llm" : "deterministic");
            }
        } catch (e) {
    ...
    async function requestAgentAction(observation) {
        ...
        if (actionResult.is_llm_generated) {
            ...
            updateModeIndicator("llm", actionResult.agent_provider);
        } else {
            logSystem(`[DETERMINISTIC] Fallback action: ${actionResult.agent_error || 'no LLM configured'}`);
            updateModeIndicator("deterministic");
        }
